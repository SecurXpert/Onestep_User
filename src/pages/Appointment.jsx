...
//     setIsLoading(true);
//     try {
//       const response = await fetchWithAuth('https://api.onestepmedi.com:8000/doctors/all', {
//         method: 'GET',
//         headers: { 'Content-Type': 'application/json' },
//       });
...
//     try {
//       const response = await fetchWithAuth(
//         `https://api.onestepmedi.com:8000/slots/available-slots?preferred_date=${date}&doctor_id=${id}`,
//         {
//           method: 'GET',
//           headers: { 'Content-Type': 'application/json' },
//         }
...
//         const filteredAvailableTimes = uniqueAvailableTimes.filter(time => {
...
//         if (filteredAvailableTimes.length === 0 && uniqueBookedTimes.length > 0) {
...
//       status: 'pending',
//       payment: selectedMode === 'Virtual' || selectedMode === 'Home Visit' ? 'completed' : 'not_required',
...
//                   />
//                 )}
//                 {selectedDate && !loadingAvailability && availableTimes.length === 0 && bookedTimes.length === 0 && (
//                   <p className="text-red-600 text-xs md:text-sm mt-2">No slots available for the selected date.</p>
//                 )}
//               </div>
...
//                 </div>
//                 <div className="flex items-center gap-2">
//                   <div className="w-3 h-3 md:w-4 md:h-4 bg-gray-200 border border-gray-300 rounded opacity-50"></div>
...
//                   {[...new Set([...availableTimes, ...bookedTimes])].sort().map((time) => {
//                     const isAvailable = availableTimes.includes(time);
...
//                             : 'bg-gray-200 border-gray-300 text-gray-500 opacity-50 cursor-not-allowed'
//                         }`}
//                       >
//                         {time}
//                       </button>
...
//                 {getAppointmentModes(doctor?.specialization_name).map((mode) => (
//                   <button
//                     key={mode}
...
//                   Available mode for {doctor.specialization_name}: {getAppointmentModes(doctor.specialization_name).join(', ')}
//                 </p>
//               )}
//             </div>
//           </div>
...
//                 {bookingStatus}
//               </p>
//               {bookingStatus.includes('Appointment booked with') && (
//                 <div className="fixed bottom-4 right-4 bg-green-600 text-white p-3 rounded shadow-lg animate-slide-up">
//                   {bookingStatus}
//                 </div>
//               )}
//             </div>
...
/** Authenticated fetch (unchanged functionality) */
const fetchWithAuth = async (url, options = {}) => {
  const token = sessionStorage.getItem("access_token");
  if (!token) {
    throw new Error("No access token found. Please log in.");
...
const Appointment = () => {
  ...
  const getAppointmentModes = (specialization) => {
    ...
    const homeVisitSpecializations = ['physiotherapist', 'nutritionist', 'homeopathy'];
    const spec = specialization?.toLowerCase() || '';
    if (offlineSpecializations.some(s => spec.includes(s))) return ['Offline'];
    if (homeVisitSpecializations.some(s => spec.includes(s))) return ['Home Visit'];
    return ['Virtual'];
  };
  ...
  const fetchRelatedDoctors = async () => {
    if (!doctor?.specialization_name || !selectedDate) return;
    try {
      const response = await fetchWithAuth(
        `https://api.onestepmedi.com:8000/slots/similar-doctors?specialization=${encodeURIComponent(
          doctor.specialization_name
  ...
  const getDayOfWeek = (offset) => {
    const date = new Date(Date.now() + offset * 86400000);
    return { label: date.toLocaleDateString('en-US', { weekday: 'short' }), date: date.toISOString().split('T')[0] };
  };

  const daysOfWeek = [getDayOfWeek(0), getDayOfWeek(1), getDayOfWeek(2), getDayOfWeek(3), getDayOfWeek(4), getDayOfWeek(5)];
  ...
  const fetchAvailableSlots = async (date) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const currentTime = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
        const filteredAvailableTimes = uniqueAvailableTimes.filter(time =>
          date === now.toISOString().split('T')[0] ? time > currentTime : true
        );
  ...
  const handleBook = async () => {
    console.log("Starting handleBook with state:", {
      id, doctor, selectedDate, selectedTime, selectedMode, patientInfo, user, bookingForSelf, reportFile,
    });
    ...
    if (!patientInfo.name) missingFields.push("Patient Name");
    if (!patientInfo.age || isNaN(parseInt(patientInfo.age, 10)) || parseInt(patientInfo.age, 10) <= 0) missingFields.push("Patient Age");
    if (!patientInfo.gender) missingFields.push("Gender");
    if (!patientInfo.phone) missingFields.push("Phone Number");
    if (!patientInfo.dob) missingFields.push("Date of Birth");
    if (!patientInfo.problem || patientInfo.problem.length < 3) missingFields.push("Problem Description (must be at least 3 characters)");
    if (!patientInfo.purpose) missingFields.push("Care Objective");
    if (missingFields.length > 0) {
      alert(`Please fill all required fields: ${missingFields.join(", ")}.`);
      return;
  ...
  if (isLoading) return <div className="text-center p-6 md:p-10">Loading...</div>;

  if (error && !doctor) {
    return <div className="text-center p-6 md:p-10 text-sm md:text-base">{error || 'Doctor not found. Please try another doctor or contact support.'}</div>;
  }

  return (
    <div className="min-h-[100dvh] bg-[#eaf2fb]">
      {/* helpers (toast animation) */}
      <style>{`
        @keyframes slideUp { from { transform: translateY(12px); opacity: 0 } to { transform: translateY(0); opacity: 1 } }
        .animate-slide-up { animation: slideUp .3s ease-out both }
      `}</style>

      {/* Wider canvas to match the mock; page uses normal scroll only */}
      <div className="mx-auto w-full max-w-[1360px] xl:max-w-[1440px] px-3 sm:px-5 lg:px-8 py-5 lg:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          {/* LEFT: Doctor card + scheduler (NO inner scroll) */}
          <section className="rounded-2xl shadow-md ring-1 ring-gray-100 overflow-hidden bg-white">
            <DoctorProfile doctor={doctor} />
            <div className="border-t border-gray-100" />
            <div className="p-4 md:p-6">
              <h3 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">Select Date & Time</h3>

              <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-semibold text-gray-800 mb-2">Select Date</label>
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {daysOfWeek.map(({ label, date }) => (
                      <button
                        key={date}
                    ...
                    <button
                      onClick={() => setCustomDateMode(true)}
                      className="px-3 py-1.5 md:px-4 md:py-2 bg-white text-indigo-600 border border-indigo-500 rounded-md text-xs md:text-sm hover:bg-indigo-50 transition"
                    >
                      Pick a Date
                    </button>
                  </div>
                  ...
                  {customDateMode && (
                    <input
                      ...
                      onChange={handleCustomDateChange}
                      min={daysOfWeek[0].date}
                      className="mt-2 md:mt-4 w-full max-w-xs px-3 py-1.5 md:px-4 md:py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-xs md:text-sm"
                    />
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2 mt-2 md:mt-0">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 md:w-4 md:h-4 bg-white border border-indigo-500 rounded" />
                    <span className="text-xs md:text-sm text-gray-600">Unselected</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 md:w-4 md:h-4 bg-indigo-600 rounded" />
                    <span className="text-xs md:text-sm text-gray-600">Selected</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 md:w-4 md:h-4 bg-gray-200 border border-gray-300 rounded opacity-60" />
                    <span className="text-xs md:text-sm text-gray-600">Not Available</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5">
                <label className="block text-xs md:text-sm font-semibold text-gray-800 mb-2">Select Time</label>
                {loadingAvailability ? (
                  <p className="text-indigo-600 text-xs md:text-sm">Loading available times...</p>
                ) : selectedDate ? (
                  <div className="grid grid-cols-2 gap-2 md:grid-cols-5 md:gap-3">
                    {[...new Set([...availableTimes, ...bookedTimes])].sort().map((time) => {
                      const isAvailable = availableTimes.includes(time);
              ...
              {/* Mode selection */}
              <div className="mt-6">
                <label className="block text-xs md:text-sm font-semibold text-gray-800 mb-2">Choose Service</label>
                <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
                  {getAppointmentModes(doctor?.specialization_name).map((mode) => (
                    <button
                      key={mode}
          ...
          {/* RIGHT: Patient Information (NO inner scroll) */}
          <section className="rounded-2xl shadow-md ring-1 ring-gray-100 bg-white p-4 md:p-6">
            <h2 className="text-2xl font-extrabold text-gray-900 mb-4 underline decoration-gray-200">Patient Information</h2>
            ...
            {loadingAvailability && (
              <p className="text-indigo-600 text-xs md:text-sm mt-4">Checking doctor availability...</p>
            )}
            ...
            {!(showRelatedDoctors || isDoctorUnavailable) && !showPayment && (
              <div className="mt-6">
                <button
                  disabled={!selectedDate || !selectedTime || !selectedMode || loadingAvailability}
                  onClick={handleBook}
                  className={`w-full max-w-[220px] md:max-w-xs p-2.5 md:p-3 rounded-lg font-semibold text-xs md:text-sm ${
                    selectedDate && selectedTime && selectedMode && !loadingAvailability
                      ? 'bg-emerald-600 text-white hover:bg-emerald-700'
                      : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  }`}
                >
                  Book Now {selectedMode && `(${selectedMode})`}
                </button>
              </div>
            )}
